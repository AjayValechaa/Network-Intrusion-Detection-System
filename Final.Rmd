...
author: "Daniel, Chetna, Akshay, Ajay"
date: "5/14/2020"
output: 
  rmdformats::material:
    code_fold: show
...
- We obeserve that the intrusions are only under `ftp`, `ftp_data`, `http`,  and `private` srvice types. The number of intrusions are higher for `private` and `http` service types.
...
- The table above suggests the outliers in `service` 8 are likely intrusions!. This is another useful interaction. 
...
> For the train data, we have an accuracy of ` r ROUND(knn.train.cm$overall["Accuracy"]*100,2) ` %, Sensitivity/ True Positive Rate of` r round(knn.train.cm$byClass["Sensitivity"]*100,2) ` %, and the 
False Positive Rate = ` r ROUND((1 - knn.train.cm$byClass["Specificity"])*100,2)` %
...
> For the test data, we have an accuracy of ` r round(knn.test.cm$overall["Accuracy"]*100) ` %, 
Sensitivity/ True Positive Rate of ` r round(knn.test.cm$byClass["Sensitivity"]*100,2) ` %, and the 
False Positive Rate = ` r round((1 - knn.test.cm$byClass["Specificity"])*100,2)` %
...
compare.table.train <- data.frame("Model"= c("Logistic","KNN", "GBM", "RF"),"True_Positive_Rate"= c(as.numeric(round(logistic.train.cm$byClass[1],4)),as.numeric(round(knn.train.cm$byClass[1],4)),round(gbm.train.sens, 4), as.numeric(round(rf.train.cm$byClass[1],4))), "False_Positive_Rate"=c(as.numeric(round(1-logistic.train.cm$byClass[2],4)),as.numeric(round(1-knn.train.cm$byClass[2],4)),round(1-gbm.train.spec,4), as.numeric(round(rf.train.cm$byClass[2],4))))
...
knn.roc
knn.auc <- knn.roc$auc
knn.roc.plot <- plot(knn.roc, print.thres = knn.auc) #plotting the roc curve with AUC highligthed
plot.roc(knn.roc.plot, col="red",print.auc=TRUE, print.auc.y=0.4,legacy.axes=TRUE, legend=TRUE)
...
> We believe the logistic model and GBM have the most promise. RF performed worse on Sensitivity. Such flase negatives can be costly, and so this model should not be used. KNN, meanwhile, is computationally expensive. Since it is not a parametric model, it must search through the dataset each time it classifies a point. This can take a long time with a large dataset.
